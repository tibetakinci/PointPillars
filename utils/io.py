...
def read_calib(file_path, extend_matrix=True):
    ...
    P0 = np.array([item for item in lines[0].split(' ')[1:]], dtype=np.float64).reshape(3, 4)
    P1 = np.array([item for item in lines[1].split(' ')[1:]], dtype=np.float64).reshape(3, 4)
    P2 = np.array([item for item in lines[2].split(' ')[1:]], dtype=np.float64).reshape(3, 4)
    P3 = np.array([item for item in lines[3].split(' ')[1:]], dtype=np.float64).reshape(3, 4)

    R0_rect = np.array([item for item in lines[4].split(' ')[1:]], dtype=np.float64).reshape(3, 3)
    Tr_velo_to_cam = np.array([item for item in lines[5].split(' ')[1:]], dtype=np.float64).reshape(3, 4)
    Tr_imu_to_velo = np.array([item for item in lines[6].split(' ')[1:]], dtype=np.float64).reshape(3, 4)
...
def read_label(file_path):
    ...
    annotation['truncated'] = np.array([line[1] for line in lines], dtype=np.float64)
    annotation['occluded'] = np.array([line[2] for line in lines], dtype=np.int64)
    annotation['alpha'] = np.array([line[3] for line in lines], dtype=np.float64)
    annotation['bbox'] = np.array([line[4:8] for line in lines], dtype=np.float64)
    annotation['dimensions'] = np.array([line[8:11] for line in lines], dtype=np.float64)[:, [2, 0, 1]] # hwl -> camera coordinates (lhw)
...
def read_label_filtered(file_path):
    ...
    annotation = {}
    annotation['name'] = np.array([line[0] for line in lines])
    annotation['dimensions'] = np.array([line[1:4] for line in lines], dtype=np.float64)
    annotation['location'] = np.array([line[4:7] for line in lines], dtype=np.float64)
    annotation['rotation_y'] = np.array([line[7] for line in lines], dtype=np.float64)
...
def write_label_filtered(result, file_path, suffix='.txt'):
    '''
    result: dict,
    file_path: str
    '''
